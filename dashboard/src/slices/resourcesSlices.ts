...
type ResourcesState = {
  ...
  resourcesId: ResourcesBasicInfo[];
...
export const updateResource = createAsyncThunk(
  "resource/updateOne",
  async (resources: any, { rejectWithValue }) => {
    const data = {
      id: resources.get("id"),
      requeriment: resources.get("requeriment"),
      description: resources.get("description"),
      price: resources.get("price"),
    };
    try {
      const options = {
        headers: {
...
export const resourcesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        getResourcesByid.fulfilled,
        (state, action: PayloadAction<ResourcesBasicInfo[]>) => {
          state.status = "idle";
          state.resourcesId = action.payload;
        }
      );
  },
});
